/** The block at the top of a record's screen, asserted on the lines it draws rather than
 *  on the props it was given: the block exists to be read, so what it is worth is what
...
/** The summary block is the first box on the screen, so its rows are the lines between the
 *  top border and the first line that is not a bordered row. */
...
/** A second and third task under the story's acceptance test, so a record has children in
 *  more than one state for the block to add up. */
...
describe("the tally the block counts with", () => {
