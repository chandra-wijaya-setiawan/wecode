//! The `tasks` row: created, erased, and amended one column at a time.
//!
//! What a task *is* gets settled once, at the save; everything after that re-declares a
//! single column of it. The narrow writes are narrow on purpose and for one reason —
//! acceptance is frozen once a task has been dispatched, and a save would carry the
//! caller's whole idea of the task back into the database beside whatever it meant to
//! change.

use rusqlite::{OptionalExtension, params};
use wecode_core::{Budget, Task, TaskId, TaskStatus};
...
mod tests {
    ...
    use wecode_core::{Budget, Number, Scope, Task, TaskId, TaskKind, TaskStatus};
