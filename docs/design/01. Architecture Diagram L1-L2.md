...
flowchart TB
    ...
    WHERE[/".wecode/workspace<br/>which workspace, so which database"/]
...
| **the files** | not a component. Three inputs, each read by the one component that needs it | `machines` `roles` `stacks` `home` |
