...
flowchart TB
    ...
    MACHINES[/"config/machines.yaml<br/>the states and every legal move"/]
    ROLES[/"config/roles.yaml<br/>the ceiling a task may not exceed"/]
...
| **the files** | not a component. Two inputs that are rules: the states and every legal move, and the ceiling a task may not exceed | `machines` `roles` |
...
| **the model is a file** | `machines.yaml` is drawn as one because that is what it is. Adding a state is an edit to data, and a move nobody declared cannot be applied |
| **only rules are drawn as inputs** | `machines.yaml` says what may happen and `roles.yaml` says how far a task may reach. Which file the database is in is inside `store`, where it belongs: it answers where, not what may happen |
