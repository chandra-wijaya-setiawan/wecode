...
flowchart TB
    ...
    subgraph core["@wecode/core"]
        ...
        SETTINGS["<b>settings</b><br/>roles · stacks · which workspace"]
    end
    ...
    LEDGER --> STORE
    VIEWS --> STORE
    SETTINGS -->|the ceiling| ENGINE
    SETTINGS -->|which file| STORE
    STORE --> DB
```

**The gate and the reads are the two halves.** A verb enters the gate and may be refused
before anything is written; a question skips it entirely. Everything else in the box serves
one or the other.
...
| **settings** | the role ceilings and invariants, stack detection, and which workspace a directory belongs to | `roles` `stacks` `home` |
