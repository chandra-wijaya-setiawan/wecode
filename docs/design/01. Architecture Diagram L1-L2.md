...
flowchart TB
    ...
    AGS["🤖 coding agent CLI<br/>system worker session"]
    ...
    WC -->|performs its own chores| AGS
    AGW -->|reports completion| WC
    AGS -->|reports completion| WC
```
...
| **system worker session** | the same, for work wecode needs done for itself — merging, refreshing a stale branch, summarising what it has learned. See [18. Chores](18.%20Chores.md) |
...
Two things you run, two that run themselves, one library they all import, and the file it
writes.
...
flowchart TB
    ...
    subgraph wecode
        ...
        DOC["@wecode/doctor<br/>«service»<br/><i>0.0.2</i>"]
...
| **@wecode/doctor** «service» | checks wecode against itself and heals what it safely can: a story delivered but never merged, a task done whose tests no longer pass, an assignment running under a runner that is gone, a worktree belonging to nothing. It also collects: worktrees nothing needs, branches already reachable from elsewhere, logs past their retention. Safe fixes and collections it does; anything that would remove the only copy of something it proposes as a `heal` or `sweep` chore that waits on your approval. **It never collects a row** — the record is history. Runs as `wecode-doctor`. **0.0.2** — see [19. Healing](19.%20Healing.md) | always, on a slow tick |
...
flowchart TB
    ...
    subgraph core["@wecode/core"]
        ...
        subgraph record["the data layer"]
            ...
            STORE["<b>store</b><br/>every row, and<br/>every transaction"]
        end
    end
...
| `runner` | daemon · allocator · foreman · examiner · git · budget · ports · adapters/claude-code · bin · index |
...
flowchart TB
    ...
    CORE -->|a test that is due| EXAM
    EXAM -->|what came back| CORE
...
| **examiner** | runs a test's artefact where the test belongs — a task_test in the tree the attempt wrote in, an acceptance_test in the story tree — and records what came back, with the provenance of the tree it ran in. It never decides what the verdict means: the gate does | the way a proof is run changes: a new kind of artefact, a new place to run it |
...
**The gate is the judge, and the examiner is not.** The examiner administers a proof and
reports what came back. Whether that verdict may be entered is the engine's decision, and a
guard may refuse it — a refused pass is not a pass, and a component that reports one as a
pass is lying about the record.
