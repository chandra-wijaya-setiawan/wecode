...
| **examiner** | runs a test's artefact where the test belongs — a task_test in the tree the attempt wrote in, an acceptance_test in the story tree — and records what came back. It never decides what the verdict means: the gate does | the way a proof is run changes: a new kind of artefact, a new place to run it |
